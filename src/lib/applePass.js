...
// counter-use copy. Discount type is black on cream — iOS Wallet forces
// primary-field text to white over a strip image, so we skip the strip.
//
...
//
// Design "Cream & botanical" (Sept 2026):
//   logo   — the hand-written "Mama" script, cut from the full logo
//   strip  — forest-green on the left fading to cream, the full wreath at
//            the right. iOS Wallet FORCES the primary field (label + value)
//            to white whenever a strip is present — so the left of the band
//            is deliberately dark, and "Member discount / 20%" sits on green.
//   icon   — wreath on cream (lock screen / notifications)
// All generated by scripts/generate-wallet-pass-images.py from
// public/mamaimg/mamalogo.png — re-run it rather than editing PNGs by hand.
...
export async function generateTribePass(card) {
  ...
  const passData = {
    ...
    // Brand cream / deep brown / forest green — same as the web card and
    // the card email. Wallet applies these to every field EXCEPT the primary
    // field over the strip, which is always white (handled by the strip art).
    backgroundColor: "rgb(249, 244, 236)",
    foregroundColor: "rgb(44, 24, 16)",
    labelColor: "rgb(31, 92, 75)",

    // No logoText — the "Mama" script logo image is the wordmark.
    ...
    storeCard: {
      ...
      // Primary sits on the strip (bottom-left) — white text on the green.
      primaryFields: [
        {
          key: "discount",
      ...
      // One short line — two used to fight for space and got truncated.
      auxiliaryFields: [
        {
          key: "counter_note",
          label: "",
          value: "Show at the counter before paying · food & drinks",
        },
      ],
      backFields: [
        ...(card.status === "expired"
