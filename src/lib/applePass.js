...
//   strip  — cream band with the full wreath at the right; the primary field
//            ("20%") is drawn by Wallet over the left of this band
//   icon   — wreath on cream (lock screen / notifications / Google logo)
...
export async function generateTribePass(card) {
  ...
  const passData = {
    ...
    // the card email. Wallet applies these globally to every field.
    ...
    storeCard: {
      ...
      // Primary sits on the strip image (bottom-left), over the cream band.
      primaryFields: [
        {
          key: "discount",
